...
;(function (root, $) {
  ...
  /**
   * Per-unit FARM-SPACE population cost (single source of truth for attack/def
   * tier classification). REAL sk104 values (get_unit_info); archer:1, marcher:5
   * for worlds that have them. Injected `opts.unitPops` overrides per world.
   ...
   * by header `order=` token (orderMap — language-proof for text-headed tables like
   * prod), or class fallback, normalizing through aliasMap. NEVER by hardcoded index.
   ...
   * @param {Object} [orderMap] - header.order token -> canonical key (text-headed tables).
   * @param {Object} [opts] - {strictAlias} — when true, ONLY the explicit aliasMap is
   *        consulted (no UNIT_HEADER_ALIASES fallback). Buildings sets this so the
   *        resource-production icons (wood/stone/iron) do NOT collide with the
   *        economy keys via UNIT_HEADER_ALIASES.
   * @returns {Object.<string, number>} key -> colIndex.
   */
  function buildColumnMap(headers, headerIconRegex, aliasMap, orderMap, opts) {
    var map = {};
    if (!Array.isArray(headers)) return map;
    aliasMap = aliasMap || {};
    orderMap = orderMap || null;
    var strictAlias = !!(opts && opts.strictAlias);
    for (var i = 0; i < headers.length; i++) {
      var h = headers[i] || {};
      var col = (h.colIndex !== undefined ? h.colIndex : i);
      // Order-token mapping (text-headed tables: prod). Highest priority — explicit.
      if (orderMap && h.order && orderMap[h.order]) {
        var okey = orderMap[h.order];
        if (map[okey] === undefined) map[okey] = col;
        continue;
      }
      var token = null;
      ...
      if (srcM && srcM[1]) {
        ...
        if (clsM) {
          ...
          for (var p = 0; p < parts.length; p++) {
            var known = aliasMap[parts[p]] || (!strictAlias && UNIT_HEADER_ALIASES[parts[p]]);
            if (parts[p] && known) {
              token = parts[p];
              break;
            }
      ...
      if (token) {
        // strictAlias: only the explicit aliasMap maps a token; unknown tokens are
        // dropped (no UNIT_HEADER_ALIASES fallback, no identity passthrough).
        var key = strictAlias
          ? aliasMap[token]
          : (aliasMap[token] || UNIT_HEADER_ALIASES[token] || token);
  ...
  function parseOverviewTable(matrix, cfg) {
    ...
    try {
      ...
      var colMap = buildColumnMap(headers, cfg.headerIconRegex, cfg.aliasMap, cfg.orderMap, cfg.columnMapOpts);
      ...
      for (var r = 0; r < rawRows.length; r++) {
        ...
        if (emitSubRows) {
          ...
          if (typeof cfg.rowPostProcess === 'function') cfg.rowPostProcess(rec, row, 0, cfg, colMap);
          out.push(rec);
          continue;
        }
        ...
        if (hasLink) {
          ...
          if (typeof cfg.rowPostProcess === 'function') cfg.rowPostProcess(data, row, cellOffset, cfg, colMap);
          out.push(data);
        }
      }
  ...
  function splitByCategory(matrix, cfg) {
    ...
    try {
      ...
      var colMap = buildColumnMap(matrix.headers || [], cfg.headerIconRegex, cfg.aliasMap, cfg.orderMap, cfg.columnMapOpts);
      var idLinkPattern = cfg.idColLinkPattern || /village=(\d+)/;
  ...
  /**
   * Parse a "a/b" pair cell (e.g. merchants "0/0" = available/total, pop "7/240" =
   * used/max) into {first, second}. Tolerates whitespace and missing halves.
   * @param {string} text
  ...
  function parsePair(text) {
    ...
    if (!m) {
      return { first: parseLocaleNumber(s), second: 0 };
    }
    return { first: parseLocaleNumber(m[1]), second: parseLocaleNumber(m[2]) };
  }

  /**
   * Per-village prod post-processor. Mutates `data`:
   *  - resources from the row's resource cell (cell.res; span.res.wood/.stone/.iron,
   *    stone->clay), defaulting to 0;
   *  - whFillPct = round(100 * max(wood,clay,iron) / whCap) when whCap > 0;
   *  - merchants "a/b" (available/total) -> merchants (total) + merchantsFree (available);
   *  - pop "u/m" (used/max) -> popUsed + popMax (and overwrites the raw `pop` key).
   * Works for the basic non-premium table too (fewer columns; missing cells -> 0).
   * @private
   */
  function prodRowPostProcess(data, row, cellOffset, cfg, colMap) {
    var cells = (row && row.cells) || [];
    cellOffset = cellOffset || 0;
    colMap = colMap || {};

    // Resources: find the cell carrying .res (the Suroviny column). Language-proof.
    var foundRes = false;
    for (var c = 0; c < cells.length; c++) {
      var res = cells[c] && cells[c].res;
      if (res && typeof res === 'object') {
        data.wood = res.wood || 0;
        data.clay = res.clay || 0;
        data.iron = res.iron || 0;
        foundRes = true;
        break;
      }
    }
    if (!foundRes) {
      if (data.wood === undefined) data.wood = 0;
      if (data.clay === undefined) data.clay = 0;
      if (data.iron === undefined) data.iron = 0;
    }

    // Merchants "a/b" -> available/total.
    if (colMap.merchants !== undefined) {
      var mCell = cells[colMap.merchants - cellOffset];
      var mp = parsePair(mCell ? mCell.text : '');
      data.merchantsFree = mp.first;
      data.merchants = mp.second;
    }

    // Pop "u/m" -> used/max.
    if (colMap.pop !== undefined) {
      var pCell = cells[colMap.pop - cellOffset];
      var pp = parsePair(pCell ? pCell.text : '');
      data.popUsed = pp.first;
      data.popMax = pp.second;
      data.pop = pp.first;
    }

    // whCap is the Sklad (storage_max) value already read by readRowData.
    var whCap = (typeof data.whCap === 'number') ? data.whCap : 0;
    if (whCap > 0) {
      var maxRes = Math.max(data.wood, data.clay, data.iron);
      data.whFillPct = Math.round((100 * maxRes) / whCap);
    } else {
      data.whFillPct = 0;
    }
  }
  ...
  var DOMAIN_CONFIGS = {
    units: {
      headerIconRegex: /unit_(\w+)\.(?:png|webp)/,
    ...
    prod: {
      // The prod (#production_table) is TEXT/order-headed, NOT icon-headed. Columns
      // are mapped by the header sort-link `order=` token (language-proof), and the
      // resources live in ONE Suroviny cell as span.res.wood/.stone/.iron — read from
      // cell.res in prodRowPostProcess, NOT from per-resource icon columns.
      headerIconRegex: null,
      aliasMap: {},
      orderMap: {
        points: 'points',          // Bodov
        storage_max: 'whCap',      // Sklad (warehouse capacity)
        trader_available: 'merchants', // Obchodníci "available/total"
        pop: 'pop'                 // Sedliacky dvor "used/max"
        // name -> identity (handled by the village-link detector, not mapped here)
      },
      idColLinkPattern: /village=(\d+)/,
      parentCategory: 'prod',
      cellReaders: {},
      rowPostProcess: prodRowPostProcess
    },
    buildings: {
      headerIconRegex: /buildings\/(\w+)\.(?:png|webp)/,
      // NOTE: resource-production buildings (wood/stone/iron = timber/clay-pit/iron-mine)
      // are intentionally OMITTED here — they would collide with the economy resource
      // keys (wood/clay/iron) in buildMasterModel's JOIN. The registry buildings columns
      // do not include resource buildings, so they are simply not mapped.
      aliasMap: {
      ...
      columnMapOpts: { strictAlias: true },
      idColLinkPattern: /village=(\d+)/,
  ...
  /**
   * Resolve a flat unit-count map from a row that may carry nested {units:{...}}
   * OR flat keys. Returns a unit accessor map (never null).
   * @private
   */
  function unitCounts(row) {
    row = row || {};
    return (row.units && typeof row.units === 'object') ? row.units : row;
  }

  /**
   * Classify the OFFENSIVE (attack) tier of a village by offensive farm-space.
   * Pure / fail-safe — never throws; bad input -> empty tier with zeroed metrics.
   *
   *   offensivePop = Σ over [axe,light,ram,catapult,(marcher if includeArchers)]
   *                  of count × pop(unit), pops from opts.unitPops || UNIT_POP.
   *   nukeFraction = offensivePop / (opts.targetNukePop || 20000).
   *   tier: 'full'    when fraction >= greenFraction(0.90) AND ram >= minRam(200)
   *         'partial' when fraction >= orangeFraction(0.40)
   *         'empty'   otherwise.
   *
   * colorKey follows opts.colorPolarity:
   *   'B' (default, offense): full=red, partial=orange, empty=green.
   *   'A' (inverted):         full=green, partial=orange, empty=red.
   *
   * @param {Object} row - village row (flat unit keys or {units}).
   * @param {Object} [opts] - {targetNukePop, greenFraction, orangeFraction, minRam,
   *                           includeArchers, colorPolarity, unitPops}.
   * @returns {{tier:string, nukeFraction:number, nukePercent:number,
   *            offensivePop:number, label:string, icon:string, colorKey:string}}
   */
  function classifyAttackTier(row, opts) {
    opts = opts || {};
    var units = unitCounts(row);
    var pops = opts.unitPops || UNIT_POP;
    var includeArchers = !!opts.includeArchers;
    var target = (opts.targetNukePop > 0) ? opts.targetNukePop : 20000;
    var green = (opts.greenFraction !== undefined && opts.greenFraction !== null) ? opts.greenFraction : 0.90;
    var orange = (opts.orangeFraction !== undefined && opts.orangeFraction !== null) ? opts.orangeFraction : 0.40;
    var minRam = (opts.minRam !== undefined && opts.minRam !== null) ? opts.minRam : 200;

    var offList = OFFENSIVE_UNITS.concat(includeArchers ? OFFENSIVE_ARCHER_UNITS : []);
    var offensivePop = 0;
    for (var i = 0; i < offList.length; i++) {
      var u = offList[i];
      offensivePop += (units[u] || 0) * (pops[u] !== undefined ? pops[u] : (UNIT_POP[u] || 0));
    }

    var fraction = target > 0 ? (offensivePop / target) : 0;
    var ram = units.ram || 0;

    var tier;
    if (fraction >= green && ram >= minRam) tier = 'full';
    else if (fraction >= orange) tier = 'partial';
    else tier = 'empty';

    var colorKey = attackColorKey(tier, opts.colorPolarity);
    var meta = ATTACK_TIER_META[tier];

    return {
      tier: tier,
      nukeFraction: fraction,
      nukePercent: Math.round(fraction * 100),
      offensivePop: offensivePop,
      label: meta.label,
      icon: meta.icon,
      colorKey: colorKey
    };
  ...
  /** tier -> {label, icon} for the attack badge. @private */
  var ATTACK_TIER_META = {
    full:    { label: 'NUKE',    icon: '⚔' },   // crossed swords
    partial: { label: 'PARTIAL', icon: '◑' },   // half-filled circle
    empty:   { label: 'EMPTY',   icon: '○' }    // empty circle
  };

  /** tier -> {label, icon} for the defense badge. @private */
  var DEF_TIER_META = {
    full:    { label: 'WALL',    icon: '⛨' },   // shield
    partial: { label: 'PARTIAL', icon: '◑' },
    empty:   { label: 'OPEN',    icon: '○' }
  };

  /**
   * Offense color polarity. 'B' (default) => full=red/partial=orange/empty=green;
   * 'A' inverts full<->empty. @private
   */
  function attackColorKey(tier, polarity) {
    var inverted = (polarity === 'A');
    if (tier === 'partial') return 'orange';
    if (tier === 'full') return inverted ? 'green' : 'red';
    return inverted ? 'red' : 'green'; // empty
  }

  /**
   * Classify the DEFENSIVE tier of a village by defensive farm-space.
   * Pure / fail-safe. Defense colorKey has a FIXED danger polarity (NOT flipped by
   * opts.colorPolarity): empty=red (danger), partial=orange, full=green.
   *
   *   defensivePop = spear + sword + heavy×6 (+ archer if includeArchers),
   *                  using opts.unitPops || UNIT_POP for the per-unit cost.
   *   defFraction  = defensivePop / (opts.targetDefPop || 20000).
   *   stacked      = defFraction >= 2.0.
   *   tier: 'full' >= greenFraction(0.90) · 'partial' >= orangeFraction(0.40) · else 'empty'.
   *
   * @param {Object} row
   * @param {Object} [opts] - {targetDefPop, greenFraction, orangeFraction,
   *                           includeArchers, unitPops}.
   * @returns {{tier:string, defFraction:number, defPercent:number,
   *            defensivePop:number, stacked:boolean, label:string, icon:string, colorKey:string}}
   */
  function classifyDefTier(row, opts) {
    opts = opts || {};
    var units = unitCounts(row);
    var pops = opts.unitPops || UNIT_POP;
    var includeArchers = !!opts.includeArchers;
    var target = (opts.targetDefPop > 0) ? opts.targetDefPop : 20000;
    var green = (opts.greenFraction !== undefined && opts.greenFraction !== null) ? opts.greenFraction : 0.90;
    var orange = (opts.orangeFraction !== undefined && opts.orangeFraction !== null) ? opts.orangeFraction : 0.40;

    var defList = DEFENSIVE_UNITS.concat(includeArchers ? DEFENSIVE_ARCHER_UNITS : []);
    var defensivePop = 0;
    for (var i = 0; i < defList.length; i++) {
      var u = defList[i];
      defensivePop += (units[u] || 0) * (pops[u] !== undefined ? pops[u] : (UNIT_POP[u] || 0));
    }

    var fraction = target > 0 ? (defensivePop / target) : 0;

    var tier;
    if (fraction >= green) tier = 'full';
    else if (fraction >= orange) tier = 'partial';
    else tier = 'empty';

    // FIXED danger polarity: empty=red, partial=orange, full=green.
    var colorKey = (tier === 'empty') ? 'red' : (tier === 'partial') ? 'orange' : 'green';
    var meta = DEF_TIER_META[tier];

    return {
      tier: tier,
      defFraction: fraction,
      defPercent: Math.round(fraction * 100),
      defensivePop: defensivePop,
      stacked: fraction >= 2.0,
      label: meta.label,
      icon: meta.icon,
      colorKey: colorKey
    };
  ...
  /**
   * Classify the offense composition of a village by offensive farm-space share.
   *   'axe-heavy' when axePop / offPop > 0.75
   *   'lc-heavy'  when lightPop / offPop > 0.55
   *   'balanced'  otherwise (or no offense).
   * @param {Object} row
   * @param {Object} [opts] - {unitPops, includeArchers}.
   * @returns {string}
   */
  function classifyOffType(row, opts) {
    opts = opts || {};
    var units = unitCounts(row);
    var pops = opts.unitPops || UNIT_POP;
    var includeArchers = !!opts.includeArchers;
    ...
    for (var i = 0; i < offList.length; i++) {
      var u = offList[i];
      offPop += (units[u] || 0) * (pops[u] !== undefined ? pops[u] : (UNIT_POP[u] || 0));
    }
    if (offPop <= 0) return 'balanced';
    var axePop = (units.axe || 0) * (pops.axe !== undefined ? pops.axe : UNIT_POP.axe);
    var lightPop = (units.light || 0) * (pops.light !== undefined ? pops.light : UNIT_POP.light);
    if (axePop / offPop > 0.75) return 'axe-heavy';
    if (lightPop / offPop > 0.55) return 'lc-heavy';
    return 'balanced';
  }
   ...
   * logic (fixes the across-buckets stale-flag bug once) AND the attack/defense
   * tier classification + curated economy/buildings/noble/scout flags.
   * @param {Object} row - A master/village row.
   * @param {Object} [opts] - Thresholds {nukeThreshold, includeArchers, defThreshold,
   *                          warnPct, fullPct, targetNukePop, greenFraction,
   *                          orangeFraction, minRam, targetDefPop, colorPolarity,
   *                          unitPops, frontBand}.
  ...
  function computeDerivedFlags(row, opts) {
    ...
    // ---- attack / defense tiers (badge columns) ----
    var atk = classifyAttackTier(out, opts);
    out.attackTier = atk;
    out.attackTierColorKey = atk.colorKey;       // table badge background
    out.nukePercent = atk.nukePercent;           // sortable numeric column
    out.offensivePop = atk.offensivePop;
    out.hasFullNuke = atk.tier === 'full';

    var def = classifyDefTier(out, opts);
    out.defTier = def;
    out.defTierColorKey = def.colorKey;
    out.defPercent = def.defPercent;
    out.defensivePop = def.defensivePop;
    out.stacked = def.stacked;

    // ---- offense composition / cats / fakes ----
    out.offType = classifyOffType(out, opts);
    var catCount = units.catapult || 0;
    out.catNuke = catCount >= 200;

    var hasAnyOffensive = false;
    var offUnitList = OFFENSIVE_UNITS.concat(includeArchers ? OFFENSIVE_ARCHER_UNITS : []);
    for (var oi = 0; oi < offUnitList.length; oi++) {
      if ((units[offUnitList[oi]] || 0) >= 1) { hasAnyOffensive = true; break; }
    }
    out.fakeAvailable = hasAnyOffensive && atk.nukeFraction < 0.40;

    // ---- noble / scout ----
    out.nobleTrainReady = (units.snob || 0) >= 4 && atk.nukeFraction >= 0.6;
    out.scoutReady = (units.spy || 0) >= 50;

    // ---- economy ----
    var popMax = (out.popMax !== undefined) ? out.popMax : 0;
    var popUsed = (out.popUsed !== undefined) ? out.popUsed : 0;
    out.freeFarmSpace = popMax - popUsed;
    out.farmCapped = popMax > 0 && popUsed >= 0.95 * popMax;
    var merchantsFreeCount = (out.merchantsFree !== undefined) ? out.merchantsFree : 0;
    out.merchantsFree = merchantsFreeCount;       // normalize (numeric)
    out.merchantsAvailable = merchantsFreeCount >= 1; // flag
    out.whFull = (fillPct !== null) && (fillPct >= 99);
    var whCap = (out.whCap !== undefined) ? out.whCap : 0;
    var minRes = Math.min(
      (out.wood || 0), (out.clay || 0), (out.iron || 0)
    );
    out.resLow = whCap > 0 && minRes < 0.10 * whCap;

    // ---- buildings ----
    var wall = (out.wall !== undefined) ? out.wall : 0;
    out.noWall = wall < 20;
    out.underBuilt = hq < 20;

    // ---- map ----
    var frontBand = (opts.frontBand !== undefined && opts.frontBand !== null) ? opts.frontBand : 25;
    var distFront = (out.distFront !== undefined && out.distFront !== null && isFinite(out.distFront))
      ? out.distFront : Infinity;
    out.frontline = distFront <= frontBand;
  ...
  /**
   * Format a tier object (or tier string) for display: 'NUKE 95%'. Reads the tier
   * object attached to the row (row[key].label / .nukePercent|.defPercent). When the
   * value is a plain string tier, uppercases it.
   * @private
   */
  function formatTier(value) {
    if (value && typeof value === 'object') {
      var pct = (value.nukePercent !== undefined) ? value.nukePercent
        : (value.defPercent !== undefined) ? value.defPercent : null;
      var label = value.label || (value.tier ? String(value.tier).toUpperCase() : '');
  ...
  /**
   * Build a BADGE column descriptor: a colored pill cell driven by the tier object
   * on the row. `tierChips` lists the tier values the table engine turns into filter
   * chips (eq on `${key}.tier`). `pctKey` is the sortable numeric % the column reads.
   * @private
   */
  function badgeCol(key, label, domain, opts) {
    opts = opts || {};
    return {
      key: key, label: label, domain: domain,
      get: opts.get || makeGetter(key),
      format: opts.format || formatTier,
      sortable: opts.sortable !== false,
      sortType: 'str',
      filterable: opts.filterable !== false,
      defaultVisible: !!opts.defaultVisible,
      ...
      render: 'badge',
      badge: true,
      tierChips: opts.tierChips || ['full', 'partial', 'empty'],
      colorKeyField: opts.colorKeyField || (key + 'ColorKey'),
      pctKey: opts.pctKey
    };
  }
  ...
  var COLUMN_REGISTRY = [
    ...
    // ---- offense tiers (badge + curated flags) ----
    badgeCol('attackTier', 'Atk tier', 'troops', { defaultVisible: true, pctKey: 'nukePercent' }),
    numCol('nukePercent', 'Nuke %', 'troops', { defaultVisible: true, format: formatPct }),
    numCol('offensivePop', 'Off pop', 'troops', {}),
    flagCol('hasFullNuke', 'Full nuke', 'troops', {}),
    strCol('offType', 'Off type', 'troops', { filterable: true }),
    flagCol('catNuke', 'Cat nuke', 'troops', {}),
    flagCol('fakeAvailable', 'Fake', 'troops', {}),

    // ---- defense tiers ----
    badgeCol('defTier', 'Def tier', 'troops', { defaultVisible: true, pctKey: 'defPercent' }),
    numCol('defPercent', 'Def %', 'troops', { format: formatPct }),
    numCol('defensivePop', 'Def pop', 'troops', {}),
    flagCol('stacked', 'Stacked', 'troops', {}),

    // ---- noble / scout ----
    flagCol('nobleTrainReady', 'Noble-ready', 'troops', {}),
    flagCol('scoutReady', 'Scout-ready', 'troops', {}),
    ...
    numCol('merchantsFree', 'Merch free', 'economy', {}),
    numCol('hideCap', 'Hide', 'economy', {}),
    numCol('freeFarmSpace', 'Free farm', 'economy', {}),
    numCol('popUsed', 'Pop', 'economy', {}),
    numCol('popMax', 'Pop max', 'economy', {}),
    flagCol('whFull', 'WH full', 'economy', {}),
    flagCol('whNearFull', 'WH near', 'economy', {}),
    flagCol('resLow', 'Res low', 'economy', {}),
    flagCol('merchantsAvailable', 'Merch avail', 'economy', {}),
    flagCol('farmCapped', 'Farm capped', 'economy', {}),
    ...
    flagCol('noWall', 'No wall', 'buildings', {}),
    flagCol('underBuilt', 'Under-built', 'buildings', {}),
    ...
    flagCol('frontline', 'Frontline', 'map', {})
  ];
  ...
  /**
   * Parse the world feature-gate flags (archer/church/watchtower/knight) out of a
   * get_config XML string. Pure / jQuery-free (string scan over the <game> block),
   * so the registry column-gating is node-testable against the REAL get_config.xml.
   * Returns numbers (0 = feature absent). Missing tags are omitted (column kept).
   * @param {string} xmlText - Raw get_config XML.
   * @returns {Object} e.g. {archer:0, church:0, watchtower:1, knight:3}.
   */
  function parseWorldConfigFlags(xmlText) {
    var flags = {};
    var xml = String(xmlText || '');
    if (!xml) return flags;
    var tags = ['archer', 'church', 'watchtower', 'knight'];
    for (var i = 0; i < tags.length; i++) {
      var tag = tags[i];
      var m = xml.match(new RegExp('<' + tag + '>\\s*(-?\\d+)\\s*</' + tag + '>'));
      if (m) flags[tag] = parseInt(m[1], 10);
    }
    return flags;
  }
  ...
  function buildMasterModel(domainData, villageIndex, opts) {
    try {
      ...
      // Enrich identity/geo from the village index. byId is the WHOLE-WORLD map index
      // (used for geo / nearest-enemy lookups), so we must ENRICH ONLY villages that
      // already have domain rows (the player's villages) — never create a master row per
      // world village (that would emit thousands of rows).
      for (var vid in byId) {
        if (!byId.hasOwnProperty(vid)) continue;
        var v = byId[vid];
        if (!v) continue;
        var mrow = merged[v.id !== undefined ? v.id : vid];
  ...
  // ============================================================
  // TECH-DEBT PURE SEAMS (M6 / T35) — the 3 fixes delegate here
  // ============================================================

  /**
   * Classify whether a units map is a nuke (own offensive village).
   * offCount = axe + light (+ marcher when settings.includeArchers). Pure; the
   * single source of truth for the Settings recalc + master-row re-flagging.
   * @param {Object} units - {axe, light, marcher, ...} (flat unit counts).
   * @param {Object} settings - {nukeThreshold, includeArchers}.
   * @returns {boolean}
   */
  function classifyNukeFlag(units, settings) {
    units = units || {};
    settings = settings || {};
    var threshold = settings.nukeThreshold || 0;
    if (threshold <= 0) return false;
    var offCount = (units.axe || 0) + (units.light || 0);
    if (settings.includeArchers) offCount += (units.marcher || 0);
    return offCount >= threshold;
  }

  /**
   * Collect the REAL localStorage cache keys the overview writes, so Clear-Cache
   * clears exactly those (and NEVER the never-written legacy troop_data_* key).
   * The prefix is the local Store prefix ('two_'); keys are still UNPREFIXED of
   * the tw-core 'twt_' tier (TWTools.Storage.remove adds that). Returns:
   *   ${p}troop_all_g<gid>, ${p}econ_g<gid>, ${p}buildings_g<gid>,
   *   ${p}incomings_g<gid> (per group) + ${p}command_data, ${p}map_villages,
   *   ${p}map_players.
   * @param {string} prefix - Local Store prefix (e.g. 'two_').
   * @param {Array.<(string|number)>} groupIds - Group ids to clear ('0' = all).
   * @returns {Array.<string>} De-duped key list.
   */
  function collectOverviewCacheKeys(prefix, groupIds) {
    var p = (prefix === undefined || prefix === null) ? '' : String(prefix);
    var keys = [];
    var seen = {};
    function push(k) { if (!seen[k]) { seen[k] = 1; keys.push(k); } }
    var gids = Array.isArray(groupIds) ? groupIds : [];
    for (var i = 0; i < gids.length; i++) {
      var g = String(gids[i]);
      push(p + 'troop_all_g' + g);
      push(p + 'econ_g' + g);
      push(p + 'buildings_g' + g);
      push(p + 'incomings_g' + g);
    }
    push(p + 'command_data');
    push(p + 'map_villages');
    push(p + 'map_players');
  ...
  /**
   * Re-flag isNuke on EVERY row of ALL 5 view buckets (own_home, in_village,
   * outside, in_transit, own_all) using classifyNukeFlag. Fixes the stale-flag
   * bug where only the visible bucket was recomputed. Mutates in place by
   * contract; tolerates missing buckets and never throws.
   * @param {Object} allTroopData - {own_home:[], in_village:[], ...} bucket map.
   * @param {Object} settings - {nukeThreshold, includeArchers}.
   * @returns {Object} The same (mutated) allTroopData.
   */
  function recomputeBucketsNuke(allTroopData, settings) {
    if (!allTroopData || typeof allTroopData !== 'object') return allTroopData;
    settings = settings || {};
    for (var c = 0; c < CATEGORY_ORDER.length; c++) {
      var bucket = allTroopData[CATEGORY_ORDER[c]];
      if (!Array.isArray(bucket)) continue;
      for (var i = 0; i < bucket.length; i++) {
        var row = bucket[i];
        if (!row || typeof row !== 'object') continue;
        row.isNuke = classifyNukeFlag(row.units || row, settings);
      }
    }
    return allTroopData;
  }
  ...
  // ============================================================
  // INCOMINGS AGGREGATION (real impl — M6 / T37)
  // ============================================================

  /**
   * Read an arrival timestamp (ms) from a command row, tolerating the several
   * field names the parser / tw-commands may emit. Infinity when unknown.
   * @private
   */
  function arrivalMsOf(c) {
    if (!c) return Infinity;
    if (typeof c.arrivalMs === 'number') return c.arrivalMs;
    if (typeof c.arrival_ms === 'number') return c.arrival_ms;
    if (typeof c.arrival === 'number') return c.arrival;
    return Infinity;
  }

  /**
   * Aggregate per-command incoming rows by TARGET village id into a per-target
   * summary, classifying each grouped train (by source) as nuke/fake/noble via
   * classifyTrainKind. Fail-safe: {} for non-array input, never throws.
   *
   * Each result entry:
   *   {
   *     count:        total incoming commands,
   *     soonestMs:    earliest arrival ms (0 when unknown),
   *     nukesEst:     # trains classified 'nuke'  (ESTIMATE),
   *     fakesEst:     # trains classified 'fake'  (ESTIMATE),
   *     noblesEst:    # trains classified 'noble' (ESTIMATE),
   *     incomingNuke: nukesEst > 0,
   *     sources:      [{coords, name, player, count}],
   *     nearestSource:{coords, name, player, dist} | null
   *   }
   *
   * @param {Array.<Object>} commands - Rows from parseOverviewTable(incomings) /
   *        tw-commands.parseIncoming (each carrying a target id).
   * @param {Object} [unitSpeeds] - Injected for the ESTIMATE classifier.
   * @param {Object} [worldConfig] - Injected for the ESTIMATE classifier.
   * @returns {Object.<string, Object>} target id -> summary.
   */
  function aggregateIncomingsByTarget(commands, unitSpeeds, worldConfig) {
    var out = {};
    if (!Array.isArray(commands)) return out;

    // Bucket commands by target id, then by source (a "train").
    var byTarget = {};
    var order = [];
    for (var i = 0; i < commands.length; i++) {
      var c = commands[i];
      var tid = (c && (c.id !== undefined && c.id !== null)) ? c.id
        : (c && (c.targetId !== undefined && c.targetId !== null)) ? c.targetId : null;
      if (tid === null) continue;
      if (!byTarget[tid]) { byTarget[tid] = []; order.push(tid); }
      byTarget[tid].push(c);
    }

    for (var t = 0; t < order.length; t++) {
      var key = order[t];
      var cmds = byTarget[key];
      var count = cmds.length;
      var soonest = Infinity;

      // Group by source coords into trains; collect source stats.
      var trains = {};
      var trainOrder = [];
      var sources = {};
      var sourceOrder = [];
      for (var j = 0; j < cmds.length; j++) {
        var cmd = cmds[j];
        var ms = arrivalMsOf(cmd);
        if (ms < soonest) soonest = ms;

        var sc = cmd.sourceCoords || cmd.origin || '';
        var grpKey = sc || ('cmd' + j); // each unknown-source command is its own train
        if (!trains[grpKey]) { trains[grpKey] = { commands: [] }; trainOrder.push(grpKey); }
        // Provide travel-time fields the ESTIMATE classifier can read.
        trains[grpKey].commands.push({
          unit: cmd.unit,
          type: cmd.cmdType || cmd.type,
          travelMs: cmd.travelMs,
          observedMs: cmd.observedMs,
          dist: (typeof cmd.distanceFloat === 'number') ? cmd.distanceFloat : cmd.distance
        });
        ...
        if (sc) {
          if (!sources[sc]) {
            sources[sc] = {
              ...
              player: cmd.player || '',
              count: 0,
              dist: (typeof cmd.distanceFloat === 'number') ? cmd.distanceFloat : (cmd.distance || 0)
            };
            sourceOrder.push(sc);
          }
          sources[sc].count++;
      ...
      var nukesEst = 0, fakesEst = 0, noblesEst = 0;
      for (var k = 0; k < trainOrder.length; k++) {
        var kind = classifyTrainKind(trains[trainOrder[k]], unitSpeeds, worldConfig);
        if (kind.kind === 'nuke') nukesEst++;
        else if (kind.kind === 'fake') fakesEst++;
        else if (kind.kind === 'noble') noblesEst++;
      }

      // Nearest source by distance.
      var nearest = null;
      var srcList = [];
      for (var s = 0; s < sourceOrder.length; s++) {
        var src = sources[sourceOrder[s]];
        srcList.push(src);
        if (!nearest || src.dist < nearest.dist) nearest = src;
      }

      out[key] = {
        count: count,
        soonestMs: (soonest === Infinity) ? 0 : soonest,
        nukesEst: nukesEst,
        fakesEst: fakesEst,
        noblesEst: noblesEst,
        incomingNuke: nukesEst > 0,
  ...
  function extractRowMatrix(html, jq) {
    ...
    try {
      ...
      // SCOPE to the overview data table — the AJAX response is the WHOLE game page
      // (many tables/menus); scanning every <tr> would grab the menu's header row and
      // map columns against the wrong header. Prefer the known overview table ids, then
      // the .overview_table / content .vis fallbacks (covers the non-premium basic table).
      var $table = $page.find('#units_table, #production_table, #buildings_table, #incomings_table').first();
      if (!$table.length) $table = $page.find('table.vis.overview_table, table.overview_table').first();
      if (!$table.length) $table = $page.find('#content_value table.vis').first();
      if (!$table.length) $table = $page.find('table.vis').first();
      var $scope = $table.length ? $table : $page;

      var $rows = $scope.find('tr');
      $rows.each(function () {
        var $tr = jq(this);
        ...
        $cells.each(function (i) {
          ...
          // Language-proof header order token, from the first sort link's order= param.
          var order = '';
          for (var li = 0; li < links.length; li++) {
            var om = (links[li].href || '').match(/[?&;]order=([a-z_]+)/i);
            if (om) { order = om[1]; break; }
          }
          // Resource spans (span.res.wood/.stone/.iron) -> {wood, clay, iron}; stone->clay.
          var res = null;
          var $res = $c.find('span.res');
          if ($res.length) {
            res = { wood: 0, clay: 0, iron: 0 };
            $res.each(function () {
              var $s = jq(this);
              var cls = ' ' + ($s.attr('class') || '') + ' ';
              var val = parseLocaleNumber(jq.trim($s.text()));
              if (cls.indexOf(' wood ') !== -1) res.wood = val;
              else if (cls.indexOf(' stone ') !== -1) res.clay = val; // stone -> clay
              else if (cls.indexOf(' clay ') !== -1) res.clay = val;
              else if (cls.indexOf(' iron ') !== -1) res.iron = val;
            });
          }
          var cell = {
            text: jq.trim($c.text()),
        ...
        if (isHeaderRow && headers.length === 0) {
          headers = cells.map(function (cell, i) {
            return {
              text: cell.text, iconSrc: cell.iconSrc, cssClass: cell.cssClass,
              colIndex: i, order: cell.order || ''
            };
          });
  ...
  var OverviewCore = {
    ...
    classifyAttackTier: classifyAttackTier,
    classifyDefTier: classifyDefTier,
    classifyOffType: classifyOffType,
    ...
    parseWorldConfigFlags: parseWorldConfigFlags,
    resolveVisibleColumns: resolveVisibleColumns,

    // tech-debt pure seams (M6 / T35)
    classifyNukeFlag: classifyNukeFlag,
    collectOverviewCacheKeys: collectOverviewCacheKeys,
    recomputeBucketsNuke: recomputeBucketsNuke,
