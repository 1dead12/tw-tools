;(function(window, $) {
  ...
  var DataFetcher = {
    ...
    /**
     * Fetch per-unit FARM-SPACE pops from /interface.php?func=get_unit_info.
     * Parses the <pop> tag per unit from the SAME XML the speeds come from.
     * Cached 24h under 'unit_pops'. Fail-safe: callback(null) on error so the
     * caller falls back to OverviewCore.UNIT_POP defaults.
     * @param {function(?Object)} callback - {unit: pop} map, or null on failure.
     */
    fetchUnitPops: function(callback) {
      var cached = Storage.get('unit_pops');
      ...
      $.ajax({
        ...
        success: function(xml) {
          ...
          $(xml).children().each(function() {
            var unitName = this.tagName;
            var pop = parseInt($(this).find('pop').text(), 10);
            if (!isNaN(pop)) pops[unitName] = pop;
          });
          // Only cache+return a non-empty map; else signal fallback.
          var hasAny = false;
          for (var k in pops) { if (pops.hasOwnProperty(k)) { hasAny = true; break; } }
          if (hasAny) {
            Storage.set('unit_pops', pops, CACHE_TTL.unitInfo);
            callback(pops);
          } else {
            callback(null);
          }
