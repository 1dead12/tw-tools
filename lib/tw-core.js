;(function(window, $) {
  ...
  // ============================================================
  // MOBILE → DESKTOP FORCE
  // ============================================================
  // TW serves a stripped-down mobile layout (body.mds) to mobile browsers.
  // Most game screens (am_farm, overview_villages, scavenge) don't work on mobile.
  // Setting the "mobile=0" cookie forces the full desktop layout on any device.
  // This cookie persists for 1 year — set once, works forever.

  (function forceDesktop() {
    if (typeof document === 'undefined') return;

    // Always ensure the cookie is set (idempotent)
    document.cookie = 'mobile=0; path=/; max-age=31536000';

    // If currently on mobile layout, reload to get desktop version
    if (document.body && document.body.classList.contains('mds')) {
      window.location.reload();
      return;
    }

    // On mobile devices viewing desktop layout, inject viewport meta for proper scaling
    // mobile_on_normal = true means "mobile device forced to desktop"
    if (typeof mobile_on_normal !== 'undefined' && mobile_on_normal === true) {
      var existing = document.querySelector('meta[name="viewport"]');
      if (existing) {
        existing.setAttribute('content', 'width=960, initial-scale=0.39, user-scalable=yes');
      } else {
        var meta = document.createElement('meta');
        meta.name = 'viewport';
        meta.content = 'width=960, initial-scale=0.39, user-scalable=yes';
  ...
  var DataFetcher = {
    ...
    fetchGroups: function(callback) {
      ...
      if (domGroups.length > 0) {
        ...
        Storage.set('village_groups', allGroups, 600000); // 10 min
        callback(allGroups);
        return;
      }

      // Method 2: AJAX fetch the overview groups page
      var villageId = getVillageId();
      $.ajax({
        url: '/game.php?village=' + villageId + '&screen=overview_villages&mode=groups',
        dataType: 'html',
        timeout: 10000,
        success: function(html) {
          ...
          self._groups = allGroups;
          Storage.set('village_groups', allGroups, 600000);
          callback(allGroups);
        },
        error: function() {
          // Fallback: just return "All villages"
          self._groups = allGroups;
          callback(allGroups);
        }
      });
