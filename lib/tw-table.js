...
;(function (root, $) {
  ...
  // Badge colorKey -> {bg, fg} pill palette (accessible: text label ALWAYS shown,
  // so the color is a redundant cue, not the sole signal).
  var BADGE_COLORS = {
    red:    { bg: '#b22222', fg: '#fff' },
    orange: { bg: '#d98a00', fg: '#fff' },
    green:  { bg: '#3a7d2c', fg: '#fff' },
    grey:   { bg: '#9a8a60', fg: '#fff' }
  };
  ...
  function buildPredicate(spec) {
    ...
    // 'tier' op: equality against the nested tier value row[key].tier (badge columns).
    // Falls back to a flat string compare when the field is not a tier object.
    if (op === 'tier') {
      return function (row) {
        if (!row) return false;
        var v = row[key];
        var t = (v && typeof v === 'object') ? v.tier : v;
        return t === value;
      };
    }
    var oc = core();
  ...
  /**
   * Build the display model for a BADGE cell (pure). Reads the tier object on the
   * row (row[col.key] = {tier,label,icon,colorKey,nukePercent|defPercent}) plus the
   * fallback colorKey field row[col.colorKeyField]. NEVER throws.
   * @param {Object} row
   * @param {Object} col - a badge column descriptor (render:'badge'|badge:true).
   * @returns {{tier:string, label:string, icon:string, colorKey:string, pct:(number|null), text:string}}
   */
  function badgeCellModel(row, col) {
    col = col || {};
    var tierObj = row ? row[col.key] : null;
    var label = '', icon = '', tier = '', colorKey = '', pct = null;
    if (tierObj && typeof tierObj === 'object') {
      tier = tierObj.tier || '';
      label = tierObj.label || (tier ? String(tier).toUpperCase() : '');
      icon = tierObj.icon || '';
      colorKey = tierObj.colorKey || '';
      if (tierObj.nukePercent !== undefined && tierObj.nukePercent !== null) pct = tierObj.nukePercent;
      else if (tierObj.defPercent !== undefined && tierObj.defPercent !== null) pct = tierObj.defPercent;
    } else if (tierObj !== undefined && tierObj !== null) {
      tier = String(tierObj);
      label = tier.toUpperCase();
    }
    // Fallback colorKey field (e.g. attackTierColorKey) when the object lacks one.
    if (!colorKey && row) {
      var ckField = col.colorKeyField || (col.key + 'ColorKey');
      if (row[ckField]) colorKey = row[ckField];
    }
    if (!colorKey) colorKey = 'grey';
    var parts = [];
    if (icon) parts.push(icon);
    if (label) parts.push(label);
    if (pct !== null) parts.push(pct + '%');
    return {
      tier: tier, label: label, icon: icon, colorKey: colorKey, pct: pct,
      text: parts.join(' ')
    };
  }
  ...
  function injectStyles(doc) {
    ...
    var css = [
      ...
      '.twt-badge { display:inline-block; padding:0 6px; border-radius:9px; font-size:10px;',
      '  font-weight:bold; line-height:15px; white-space:nowrap; }'
    ].join('\n');
  ...
  function render(container, model, opts) {
    ...
    function rowHtml(row) {
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        var col = cell.col;
        var inner;
        if (col && (col.render === 'badge' || col.badge)) {
          var bm = badgeCellModel(row, col);
          var pal = BADGE_COLORS[bm.colorKey] || BADGE_COLORS.grey;
          inner = '<span class="twt-badge" style="background:' + pal.bg + ';color:' + pal.fg +
            ';">' + escapeHtml(bm.text) + '</span>';
        } else if (col && col.isCoordCol && typeof col.coordGet === 'function') {
  ...
  function collectChipSpecs(columns) {
    ...
    for (var i = 0; i < columns.length; i++) {
      var col = columns[i];
      if (!col) continue;
      // Threshold chips (numeric gte/lte/...).
      if (Array.isArray(col.thresholds)) {
      ...
      // Tier chips (one per tier value) for badge columns.
      if ((col.render === 'badge' || col.badge) && Array.isArray(col.tierChips)) {
        for (var c = 0; c < col.tierChips.length; c++) {
          var tv = col.tierChips[c];
          out.push({
            key: col.key, op: 'tier', value: tv,
            label: (col.label || col.key) + ': ' + String(tv)
          });
        }
      }
    }
