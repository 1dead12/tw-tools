;(function(window, $) {
  ...
  function createCard(options) {
    ...
    // Apply size and position — validate saved values against current viewport
    // so a card stored at a position that's now off-screen still renders visibly.
    function isFiniteNum(n) { return typeof n === 'number' && isFinite(n); }
    var validSaved = savedPos &&
      isFiniteNum(savedPos.left) && isFiniteNum(savedPos.top) &&
      isFiniteNum(savedPos.width) && isFiniteNum(savedPos.height);

    var curWidth, curHeight, posLeft, posTop;
    if (validSaved) {
      curWidth = Math.max(minWidth, Math.min(savedPos.width, window.innerWidth));
      curHeight = Math.max(minHeight, Math.min(savedPos.height, window.innerHeight));
      // Keep at least 80px of card visible horizontally and the full header bar visible vertically.
      var maxLeft = Math.max(0, window.innerWidth - 80);
      var maxTop = Math.max(0, window.innerHeight - 30);
      posLeft = Math.max(0, Math.min(savedPos.left, maxLeft));
      posTop = Math.max(0, Math.min(savedPos.top, maxTop));
    } else {
      curWidth = width;
      curHeight = height;
      posLeft = Math.max(10, (window.innerWidth - width) / 2);
      posTop = Math.max(10, (window.innerHeight - height) / 2);
    }
    ...
    function savePosition() {
      ...
      // Skip saving degenerate values (e.g. card hidden/minimized) — they
      // would later restore as an invisible 0×0 card stuck in a corner.
      if (!w || !h || w < minWidth || h < minHeight) return;
    ...
    // ---- Destroy ----
    function doDestroy() {
      // Save position before destroying so it can be restored on reopen
      savePosition();
      destroyed = true;
      if (timeInterval) {
    ...
    return {
      ...
      /**
       * Set a tab's content panel HTML.
       * @param {string} tabId - Tab identifier.
       * @param {string} html - HTML content to set.
       */
      setTabContent: function(tabId, html) {
        var $panel = tabPanels[tabId];
        if ($panel) $panel.html(html);
      },
