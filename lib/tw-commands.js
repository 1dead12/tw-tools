;(function(window, $) {
  ...
  function parseIncoming() {
    ...
    $table.find('tbody tr, tr').each(function() {
      ...
      // String-form source coords for per-target aggregation grouping (additive).
      var srcCoordsMatch = originText.match(/(\d{1,3}\|\d{1,3})/);
      cmd.sourceCoords = srcCoordsMatch ? srcCoordsMatch[1] : '';

      // TARGET village (Cieľ column) — additive: emit target id + coords/name so
      // the overview can aggregate incomings PER target village. Existing
      // source/origin/arrival fields and the CMD_TYPE vocabulary are unchanged.
      var $targetCell = $cells.eq(colMap.target);
      var targetText = $targetCell.text().trim();
      var $targetLink = $targetCell.find('a[href*="info_village"], a[href*="village="]').first();
      var targetHref = $targetLink.attr('href') || '';
      var targetIdMatch = targetHref.match(/(?:id|village)=(\d+)/);
      if (targetIdMatch) cmd.targetId = parseInt(targetIdMatch[1], 10);
      var targetCoordsMatch = targetText.match(/(\d{1,3}\|\d{1,3})/);
      cmd.targetCoords = targetCoordsMatch ? targetCoordsMatch[1] : '';
      cmd.targetName = targetText.replace(/\s*\(\d{1,3}\|\d{1,3}\)\s*K?\d*\s*$/, '').trim();
      cmd.target = TWTools.parseCoords(targetText) || cmd.target;
